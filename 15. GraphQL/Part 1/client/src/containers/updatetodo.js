...
class AddtoDo extends React.Component {
    ...
    async submitForm(e){
        ...
        await ApiService.updateTodos({id: this.state.todo_id});
        ...
        window.location.replace("http://localhost:3000/updatetodo");
    }
    ...
    display(){
        ...
        return this.state.todo_list.map(todo=>{
              if(todo.completed ===false)return (<option key = {todo.id} value = {todo.id}>{todo.title}</option>)
        })
    }
